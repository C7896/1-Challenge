...
// Today's completed flag and streak, keyed by uid and calendar day. The flag tab
// used to block on two Firestore reads before it navigated anywhere, which read
// as lag. Home already loads this when it comes into focus, so by the time the
// tab is tapped the answer is usually already here.
let today = { uid: null, key: null, value: null };
...
export function clearCachedUser() {
    ...
    today = { uid: null, key: null, value: null };
}

export function getCachedToday(uid, key) {
    return today.uid === uid && today.key === key ? today.value : null;
}

export function setCachedToday(uid, key, value) {
    today = { uid, key, value };
}
